...
static void update_display(void)
{
    ...
    snprintf(item_keys[6], sizeof(item_keys[6]), "%s", i18n(STR_RESET_COUNT));
    ...
    if (pomodoro_list) {
        if (pomo_mode == POMO_MODE_ADJUST) {
            ui_list_set_selected_color(pomodoro_list, UI_COLOR_ACCENT);
        } else {
            ui_list_set_selected_color(pomodoro_list, UI_COLOR_SUCCESS);
        }
...
lv_obj_t* ui_screen_settings_pomodoro_create(void)
{
    ...
    lv_obj_t *title = ui_create_title_label(screen, i18n(STR_T_POMODORO));
