...
void aht20_init(void)
{
    ...
    i2c_master_transmit(dev_handle, &reset_cmd, 1, -1);
    vTaskDelay(pdMS_TO_TICKS(20));

    /* Calibration: enable CRC + output (status bit[3]=1, bit[2]=1) */
    uint8_t init_cmd[] = {AHT20_CMD_INIT, 0x08, 0x00};
    i2c_master_transmit(dev_handle, init_cmd, sizeof(init_cmd), -1);
...
bool aht20_read(float *temperature, float *humidity)
{
    ...
    esp_err_t ret = i2c_master_transmit(dev_handle, trigger_cmd, sizeof(trigger_cmd), -1);
    if (ret != ESP_OK) {
